...
passage_num2 <- read.csv("passage_num2.csv", stringsAsFactors = FALSE, header = TRUE, sep = ";")
passage_num3 <- read.csv("passage_num3.csv", stringsAsFactors = FALSE, header = TRUE, sep = ";")
#cleaning function
clean_evtype <- function(subject = "evtype_data", passage_num = 1) {
      ...
      for(j in 1:passage_num) {
            ...
            evtype_clean[ , cond_title] <- NA
            for(i in 1:length(cond$grep)) {
                  index <- grep(cond$grep[i], evtype_clean$evtype, ignore.case = TRUE)
                  if(length(index > 0)) {
                  for(z in 1:length(index)) { evtype_clean[index[z], cond_title] <- cond$output[i]                    
...
clean_evtype(passage_num = 2)
clean_evtype(passage_num = 3)
