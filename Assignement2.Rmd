...
clean_evtype <- function(subject = "evtype_data", passage_num = 1) {
      ...
      for(j in 1:passage_num) {
            ...
            ifelse(j == 1, evtype_clean[ , cond_title] <- NA, evtype_clean[ , cond_title] <- evtype_clean[ , cond_title_previous])
            for(i in 1:length(cond$grep)) {
                  ...
                  for(z in 1:length(index)) {
                        if(j == 1) {
                              ...
                              evtype_clean[index[z], cond_title] <- cond$output[i]
                              }
                        else { 
                              
                              
                              if(is.na(evtype_clean[index[z], cond_title_previous])){
